...
class SelectComprar(ui.Select):
    def __init__(self, itens, view):
        ...
        for id_item, nome, tipo, preco, estoque, efeito in itens:
            self.view_pai.cache[str(id_item)] = {"id":id_item, "nome":nome, "tipo":tipo, "preco":preco, "estoque":estoque, "efeito":efeito}
...
class LojaView(ui.View):
    ...
    async def atualizar_comprar(self, i):
        ...
        if itens: self.add_item(SelectComprar(itens[:25], self))
...
class Shop(commands.Cog):
    ...
    async def loja(self, i: discord.Interaction):
        async with self.bot.db.execute("SELECT id FROM personagens WHERE user_id=?", (i.user.id,)) as c:
            if not await c.fetchone(): return await i.response.send_message("❌ Crie ficha.", ephemeral=True)
        view = LojaView(self.bot.db, i.user.id)
